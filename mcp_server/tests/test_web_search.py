...
class ProviderDispatchTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dispatches_to_puter_when_overridden(self) -> None:
        expected = ("answer", {1: "content"}, {1: "https://example.com"})
        with patch(
            "src.app.web_search_handler.run_puter_search",
            new=AsyncMock(return_value=expected),
        ) as search:
            result = await run_web_search("query", provider="puter")

        self.assertEqual(result, expected)
        search.assert_awaited_once_with("query")


class PuterResponseTests(unittest.TestCase):
    def test_parse_raw_puter_text_extracts_urls(self) -> None:
        from src.app.puter_handler import parse_raw_puter_text

        sample_text = "According to https://python.org and https://fastmcp.com, this is how it works."
        full_answer, answers, citations = parse_raw_puter_text(sample_text)

        self.assertEqual(len(citations), 2)
        self.assertEqual(citations[1], "https://python.org")
        self.assertEqual(citations[2], "https://fastmcp.com")
        self.assertIn("### [1]: https://python.org", full_answer)
