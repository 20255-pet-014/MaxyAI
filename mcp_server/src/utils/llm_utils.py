...
def get_chat_model(model_id: str, schema: Optional[Type[BaseModel]] = None) -> BaseChatModel | TrackedLangChainModel:
    ...
    if api_key_env_var:
        ...
        api_key = None
        if api_key_env_var == "GROQ_API_KEY" and settings.groq_api_key:
            api_key = settings.groq_api_key.get_secret_value()
        elif api_key_env_var == "GOOGLE_API_KEY" and settings.google_api_key:
    ...
    if schema is not None:
        if api_key_env_var == "GROQ_API_KEY" or "groq" in init_kwargs.get("base_url", ""):
            model = model.with_structured_output(schema, method="function_calling")
        else:
