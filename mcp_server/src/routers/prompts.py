...
from ..prompts.writer_instructions_prompt import full_writer_instructions_prompt as _get_writer_instructions
...
def register_mcp_prompts(mcp: FastMCP) -> None:
    ...
    async def full_research_instructions_prompt() -> str:
        """Complete Maxy research agent workflow instructions."""
    ...
    async def full_writer_instructions_prompt() -> str:
        """Complete Maxy Writer & Critic agent workflow instructions."""

        opik_context.initialize_thread_id()

        return await _get_writer_instructions()
