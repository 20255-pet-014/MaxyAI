...
def register_mcp_tools(mcp: FastMCP) -> None:
    ...
    # ============================================================================
    # WRITER AND CRITIC AGENT TOOLS
    # ============================================================================

    @mcp.tool()
    @opik.track(type="tool")
    async def read_research_summary(research_directory: str) -> Dict[str, Any]:
        """
        Read and extract key metadata, guidelines, and source references from research.md.

        Args:
            research_directory: Path to the research directory containing research.md

        Returns:
            Dict containing guidelines, character counts, and cited source URLs.
        """
        opik_context.update_thread_id()
        return await read_research_summary_tool(research_directory)

    @mcp.tool()
    @opik.track(type="tool")
    async def generate_article_outline(research_directory: str) -> Dict[str, Any]:
        """
        ...
        """
        opik_context.update_thread_id()
        return await generate_article_outline_tool(research_directory)

    @mcp.tool()
    @opik.track(type="tool")
    async def draft_article_section(
        research_directory: str,
        section_title: str,
        key_points: List[str],
        ...
        """
        opik_context.update_thread_id()
        return await draft_article_section_tool(
    ...
    @mcp.tool()
    @opik.track(type="tool")
    async def critic_evaluate_article(
        research_directory: str,
        article_content: str,
    ) -> Dict[str, Any]:
        """
        ...
        """
        opik_context.update_thread_id()
        return await critic_evaluate_article_tool(
            research_directory=research_directory,
    ...
    @mcp.tool()
    @opik.track(type="tool")
    async def refine_and_save_article(
        research_directory: str,
        article_content: str,
        ...
        """
        opik_context.update_thread_id()
        return await refine_and_save_article_tool(
            research_directory=research_directory,
            article_content=article_content,
    ...
    # ============================================================================
    # BUILDER AND PUBLISHER AGENT TOOLS (PHASE 3)
    # ============================================================================

    @mcp.tool()
    @opik.track(type="tool")
    async def extract_and_validate_code(research_directory: str) -> Dict[str, Any]:
        """
        Extract all code blocks from article.md, validate their AST syntax, and store in .nova/sandbox/.

        Args:
            research_directory: Path to the research directory containing article.md

        Returns:
            Dict containing counts of valid and invalid code blocks, list of snippets, and sandbox location.
        """
        opik_context.update_thread_id()
        return await extract_and_validate_code_tool(research_directory)

    @mcp.tool()
    @opik.track(type="tool")
    async def execute_sandbox_tests(research_directory: str) -> Dict[str, Any]:
        """
        ...
        """
        opik_context.update_thread_id()
        return await execute_sandbox_tests_tool(research_directory)

    @mcp.tool()
    @opik.track(type="tool")
    async def fix_code_snippets(
        research_directory: str,
        broken_code: str,
        error_message: str,
        language: str = "python",
        ...
        Args:
            research_directory: Path to the research directory
            broken_code: The raw code string that failed execution or AST validation
            error_message: The error or traceback message
        ...
        """
        opik_context.update_thread_id()
        return await fix_code_snippets_tool(
            research_directory=research_directory,
    ...
    @mcp.tool()
    @opik.track(type="tool")
    async def export_publication_bundle(
        research_directory: str,
        ...
        """
        opik_context.update_thread_id()
        return await export_publication_bundle_tool(
            research_directory=research_directory,
