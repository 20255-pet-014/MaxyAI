...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_file=(".env", "mcp_server/.env", "../mcp_server/.env", "mcp_client/.env", "../mcp_client/.env"),
        extra="ignore",
        env_file_encoding="utf-8",
    )

    # Server settings
    server_name: str = Field(default="Maxy Research MCP Server", description="The name of the server")
    ...
    scraping_model: str = Field(default="llama-3.1-8b", description="Model for web scraping")
    query_generation_model: str = Field(default="llama-3.1-8b", description="Model for query generation")
    source_selection_model: str = Field(default="llama-3.1-8b", description="Model for source selection")

    # Web search configuration
    web_search_provider: Literal["tavily", "perplexity", "puter"] = Field(
        default="perplexity",
    ...
    puter_model: str = Field(
        default="perplexity/sonar",
        alias="PUTER_MODEL",
        description="Model to use for Puter AI search (e.g. perplexity/sonar, perplexity/sonar-pro)",
    )
    ...
    puter_auth_token: SecretStr | None = Field(default=None, alias="PUTER_AUTH_TOKEN", description="The Auth token for Puter.js AI API")
    ...
    opik_project_name: str = Field(default="maxy", alias="OPIK_PROJECT_NAME", description="Opik's project name")
